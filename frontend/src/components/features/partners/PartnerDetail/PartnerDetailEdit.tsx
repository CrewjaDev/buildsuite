...
import { Partner, UpdatePartnerRequest } from '@/types/features/partners/partner'
import { useUpdatePartner } from '@/hooks/features/partners/usePartners'

interface PartnerDetailEditProps {
  partner: Partner
  onCancel: () => void
  onSuccess: () => void
}

export function PartnerDetailEdit({ partner, onCancel, onSuccess }: PartnerDetailEditProps) {
  ...
  const [formData, setFormData] = useState<UpdatePartnerRequest>({
    ...
    closing_date: partner.closing_date || undefined,
    deposit_terms: partner.deposit_terms || '',
    deposit_date: partner.deposit_date || undefined,
    deposit_method: partner.deposit_method || '',
    cash_allocation: partner.cash_allocation || undefined,
    bill_allocation: partner.bill_allocation || undefined,
    payment_date: partner.payment_date || undefined,
    payment_method: partner.payment_method || '',
    payment_cash_allocation: partner.payment_cash_allocation || undefined,
    payment_bill_allocation: partner.payment_bill_allocation || undefined,
    establishment_date: partner.establishment_date || '',
    capital_stock: partner.capital_stock || undefined,
    previous_sales: partner.previous_sales || undefined,
    employee_count: partner.employee_count || undefined,
    business_description: partner.business_description || '',
    bank_name: partner.bank_name || '',
    branch_name_bank: partner.branch_name_bank || '',
    account_type: partner.account_type || undefined,
  ...
  const handleInputChange = (field: keyof UpdatePartnerRequest, value: string | number | boolean | undefined) => {
    setFormData(prev => ({
      ...prev,
      [field]: value
    }))
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="flex justify-end space-x-2">
        ...
        <Button type="submit" disabled={updatePartner.isPending}>
          {updatePartner.isPending ? '更新中...' : '更新'}
        </Button>
      </div>
    </form>
  )
