...
class UniversalApprovalService
{
    ...
    public function createApprovalRequest(
        ...
        // 承認依頼可能かチェック
        if (!$data->canRequestApproval($user)) {
            throw new ApprovalException('承認依頼を作成できません');
        }

        // 承認依頼の作成
        return $data->createApprovalRequest($user, $flowId);
    }
     ...
     * 業務データの取得
     */
    private function getBusinessData(string $type, int $id): ?ApprovableData
    {
        return match($type) {
...
class ApprovalException extends \Exception
{
    public function __construct(
        string $message,
        public readonly string $code = 'APPROVAL_ERROR',
